...
class DataGenerator:
        ...
        try:
            for _ in range(count):
                ...
                new_user = User(
                    ...
                    phone_number=fake.phone_number()[:20],  # LIMIT LENGTH
    ...
    def create_products(self, db: Session, categories: List[Category], count: int) -> List[Product]:
        products = []
        batch_size = 100  # 👈 Commit every 100 products
        ...
        try:
            for i in range(count):
                ...
                if (i + 1) % batch_size == 0:
                    db.commit()
                    print(f"✅ Committed {i + 1} products so far...")

            db.commit()  # Final commit
            print(f"✅ Final commit: total {len(products)} products.")
            return products
