...
def get_user_orders(user_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    return order_crud.get_user_orders(db, user_id)  # Fixed duplicate function call issue
 
 
@router.get("/{user_id}/products")
def get_all_products_route(user_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if current_user.user_id != user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")
...
def get_user_cart_route(user_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if current_user.user_id != user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")
    return get_user_cart(db, user_id)  # added db argument
